...
inhomogeneous_sigma = 0.2

e = np.arange(0,2.01,0.05)
signal = []
...
e1_ind = len(signal)//2 #midpoint of evenly spaced energies
print(e[e1_ind])

integ_weights = 1/(np.sqrt(2*np.pi)*inhomogeneous_sigma)*np.exp(-(e-1)**2/(2*inhomogeneous_sigma**2))

true_signal = signal[e1_ind] * np.exp(-(t[np.newaxis,:]-tau[:,np.newaxis])**2/(2*1/inhomogeneous_sigma**2))
...
for i in range(e1_ind+1):
    x = e[e1_ind-i:e1_ind+i+1]
    weights = integ_weights[e1_ind-i:e1_ind+i+1]
    y = np.array(signal[e1_ind-i:e1_ind+i+1]) * weights[:,np.newaxis,np.newaxis]
    averaged_signal = np.trapz(y, x=x, axis = 0)
    diffs.append(L2_norm(averaged_signal,true_signal))

num_simulations = np.arange(e1_ind+1)*2+1
plt.figure()
...
signal = np.trapz(np.array(signal)*integ_weights[:,np.newaxis,np.newaxis], x = e, axis = 0)
